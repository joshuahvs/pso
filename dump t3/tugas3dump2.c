...
int main(int argc, char *argv[]) {
    setbuf(stdout, NULL); // Set stdout to unbuffered mode
...
void *seller_thread(void *arg) {
    ...
    while (1) {
        // Wait for supplier to supply items
        sem_wait(&sem_stock);
        ...
        // Wait for notification from buyer that warehouse is empty
        sem_wait(&sem_order);
        ...
        // Order items from supplier again
        pthread_mutex_lock(&print_mutex);
        printf("Seller: stok barang HABIS, order dulu ke supplier\n");
        pthread_mutex_unlock(&print_mutex);
        sem_post(&sem_supply);
