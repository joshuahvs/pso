...
pthread_mutex_t warehouse_mutex = PTHREAD_MUTEX_INITIALIZER; // Mutex for warehouse_empty_flag

// Flag to indicate if warehouse is empty
...
void *buyer_thread(void *arg) {
    ...
    if (buyers_remaining == 0) {
        ...
        sem_post(&sem_stock);
        sem_post(&sem_supply);
        // Signal any buyers waiting on sem_available
        sem_post(&sem_available);
    }
    pthread_mutex_unlock(&buyer_mutex);
